...
func main() {
	...
	if *contractAddressString == "" {
		...
		nonce, err := GetNonce(client, *privateKey) // Replace with the nonce of the account
		...
		txn, err := CallContractMethod(client, *privateKey, contract, *methodName, *methodArgs, nonce, contractAddress, amount, gasLimit, big.NewInt(gasPrice))
		if err != nil {
...
func CallContractMethod(client EthClient, privateKeyHex string, contract *compiler.Contract, methodName string, args string,
