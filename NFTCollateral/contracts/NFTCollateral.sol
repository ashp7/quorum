...
contract NFTCollateralLoan {
    ...
    // Event declarations
    event ProposalSubmitted(uint256 proposalId, address borrower, uint256 loanAmount);
    event ProposalAccepted(uint256 proposalId, address lender);
    event LoanRepaid(uint256 proposalId);
    event ProposalRetracted(uint256 proposalId);
    event LoanDateExpired(uint256 proposalId);
    ...
    function submitProposal(address _nftContractAddress, uint256 _nftTokenId, uint256 _loanAmount, uint256 _interestRate, uint256 _duration) external {
        ...
        emit ProposalSubmitted(nextProposalId, newProposal.borrower, newProposal.loanAmount);
    ...
    function acceptProposal(uint256 _proposalId) external payable {
        ...
        emit ProposalAccepted(nextProposalId, proposal.lender);
    }
    ...
    function retractProposal(uint256 _proposalId) external {
        ...
        emit ProposalRetracted(_proposalId);
    }
